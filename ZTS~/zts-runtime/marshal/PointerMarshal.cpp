...
{
    JSValue flag = JS_GetPropertyStr(ctx, value, "__zts_pointer");
...
JSValue PointerMarshal::Push(JSContext* ctx, void* address, const Il2CppType* ptrType)
{
    ...
    JS_SetPropertyStr(ctx, obj, "__zts_id", JS_NewInt32(ctx, (int32_t)slot));
    JS_SetPropertyStr(ctx, obj, "__zts_pointer", JS_NewBool(ctx, 1));
    return obj;
}
...
void* PointerMarshal::Pop(JSContext* ctx, JSValueConst value, const Il2CppType* expectedPtrType)
{
    ...
    {
        JS_ThrowTypeError(ctx, "zts: undefined is not assignable to Pointer (use null for null pointer)");
        return nullptr;
    }

    if (!JS_IsObject(value) || !HasPointerFlag(ctx, value))
    {
        JS_ThrowTypeError(ctx, "zts: expected Pointer handle");
        return nullptr;
    }
    ...
    {
        JS_ThrowTypeError(ctx, "zts: Pointer type mismatch");
        return nullptr;
    }
