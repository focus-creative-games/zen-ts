...
{
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "zts: ensure_assembly requires a name");
    std::string name = ReadStringArg(ctx, argv, 0);
    if (name.empty())
        return JS_EXCEPTION;
    if (MetadataUtil::ResolveAssembly(name.c_str()) == nullptr)
        return JS_ThrowReferenceError(ctx, "zts: assembly not found: %s", name.c_str());
    return JS_UNDEFINED;
}
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: resolve_type requires assembly and type name");
    ...
    if (assembly == nullptr)
        return JS_ThrowReferenceError(ctx, "zts: assembly not found: %s", asmName.c_str());

    Il2CppClass* klass = MetadataUtil::ResolveType(assembly, typeName.c_str());
    if (klass == nullptr)
        return JS_ThrowReferenceError(ctx, "zts: type not found: %s", typeName.c_str());
...
{
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "zts: typeof expects a type object");
    Il2CppClass* klass = KlassFromTypeObject(ctx, argv[0]);
    if (klass == nullptr && ObjectRegistry::IsZtsObject(ctx, argv[0]))
    {
    ...
    JS_SetPropertyStr(ctx, handle, "__zts_type_name", JS_NewString(ctx, fullName.c_str()));
    return handle;
}
...
{
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "zts: make_szarray_type requires element type");
    Il2CppClass* element = KlassFromTypeObject(ctx, argv[0]);
    if (element == nullptr)
        return JS_ThrowTypeError(ctx, "zts: make_szarray_type expects a type object");
    Il2CppClass* arrayKlass = il2cpp::vm::Class::GetArrayClass(element, 1);
    if (arrayKlass == nullptr)
        return JS_ThrowInternalError(ctx, "zts: failed to create szarray type");
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: new_szarray_by_element_type requires type and length");
    Il2CppClass* element = KlassFromTypeObject(ctx, argv[0]);
    if (element == nullptr)
        return JS_ThrowTypeError(ctx, "zts: expected element type");
    int32_t length = 0;
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: new_szarray_by_szarray_type requires type and length");
    Il2CppClass* arrayKlass = KlassFromTypeObject(ctx, argv[0]);
    if (arrayKlass == nullptr || arrayKlass->rank != 1)
        return JS_ThrowTypeError(ctx, "zts: expected szarray type");
    int32_t length = 0;
    if (JS_ToInt32(ctx, &length, argv[1]))
        return JS_EXCEPTION;
    if (length < 0)
...
static JSValue ToArrayCallback(JSContext* ctx, JSValueConst /*this_val*/, int argc, JSValueConst* argv)
{
    ...
    if (obj == nullptr || obj->klass->rank == 0)
        return JS_ThrowTypeError(ctx, "zts: to_array expects a CLR szarray");
...
{
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "zts: get_type_from_name requires a name");
    std::string name = ReadStringArg(ctx, argv, 0);
    if (name.empty())
        return JS_ThrowReferenceError(ctx, "zts: type not found: ");

    Il2CppClass* klass = MetadataUtil::ResolveTypeByName(name.c_str());
    if (klass == nullptr)
        return JS_ThrowReferenceError(ctx, "zts: type not found: %s", name.c_str());
    return TypeRegistry::PushTypeObject(ctx, klass);
}
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: cast requires value and type");
    Il2CppClass* klass = KlassFromTypeObject(ctx, argv[1]);
    if (klass == nullptr)
        return JS_ThrowTypeError(ctx, "zts: cast target is not a type");

    const MarshalMetaInfo* meta = MarshalMeta::TryCreateDefault(&klass->byval_arg);
    if (meta == nullptr)
        return JS_ThrowTypeError(ctx, "zts: cast type not supported yet");
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: box requires type and value");
    Il2CppClass* klass = KlassFromTypeObject(ctx, argv[0]);
    if (klass == nullptr)
        return JS_ThrowTypeError(ctx, "zts: box expects a type object");
    if (!il2cpp::vm::Class::IsValuetype(klass))
        return JS_ThrowTypeError(ctx, "zts: box expects a value type or enum");

    const MarshalMetaInfo* meta = MarshalMeta::TryCreateDefault(&klass->byval_arg);
    if (meta == nullptr)
        return JS_ThrowTypeError(ctx, "zts: box type not supported yet");
...
static JSValue UnboxCallback(JSContext* ctx, JSValueConst /*this_val*/, int argc, JSValueConst* argv)
{
    ...
    Il2CppObject* obj = ObjectRegistry::Get(ctx, argv[0]);
    if (obj == nullptr)
        return JS_ThrowTypeError(ctx, "zts: unbox expects a CLR object");

    Il2CppClass* klass = obj->klass;
    const MarshalMetaInfo* meta = MarshalMeta::TryCreateDefault(&klass->byval_arg);
    if (meta == nullptr)
        return JS_ThrowTypeError(ctx, "zts: unbox type not supported yet");
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: make_generic_type requires open type and args");
    Il2CppClass* open = KlassFromTypeObject(ctx, argv[0]);
    if (open == nullptr || !open->is_generic)
        return JS_ThrowTypeError(ctx, "zts: make_generic_type expects an open generic type");

    JSValue argsVal = argv[1];
    if (!JS_IsArray(ctx, argsVal))
        return JS_ThrowTypeError(ctx, "zts: make_generic_type args must be an array");
    ...
    JS_FreeValue(ctx, lenVal);
    if (count <= 0)
        return JS_ThrowTypeError(ctx, "zts: make_generic_type requires type arguments");
    ...
    for (int32_t i = 0; i < count; ++i)
    {
        ...
        if (argKlass == nullptr)
            return JS_ThrowTypeError(ctx, "zts: invalid generic type argument");
        types[i] = &argKlass->byval_arg;
    }

    Il2CppClass* inflated = il2cpp::vm::Class::GetInflatedGenericInstanceClass(open, types, (uint32_t)count);
    if (inflated == nullptr)
        return JS_ThrowInternalError(ctx, "zts: failed to inflate generic type");
...
{
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "zts: to_bytes expects CLR array");
    Il2CppObject* obj = ObjectRegistry::Get(ctx, argv[0]);
    if (obj == nullptr || obj->klass->rank == 0)
        return JS_ThrowTypeError(ctx, "zts: to_bytes expects CLR array");
    ...
    if (element == il2cpp_defaults.boolean_class || element == il2cpp_defaults.char_class
        ...
        return JS_ThrowTypeError(ctx, "zts: to_bytes: element type is not blittable");
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: to_delegate requires function and delegate type");
    if (!JS_IsFunction(ctx, argv[0]))
        return JS_ThrowTypeError(ctx, "zts: to_delegate expects a function");
    Il2CppClass* delKlass = KlassFromTypeObject(ctx, argv[1]);
    if (delKlass == nullptr || !MetadataUtil::IsDelegateClass(delKlass))
        return JS_ThrowTypeError(ctx, "zts: to_delegate expects delegate type");
    ...
    if (del == nullptr)
    {
        ...
        return JS_ThrowTypeError(ctx, "zts: failed to create delegate");
    }
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: register_method requires name and function");
    std::string name = ReadStringArg(ctx, argv, 0);
    if (name.empty())
        return JS_ThrowTypeError(ctx, "zts: register_method requires a non-empty name");

    MethodMarshalCtx* mctx = MetaBinding::TryGetDirectMethodCtx(ctx, argv[1]);
    if (mctx == nullptr || mctx->method == nullptr)
        return JS_ThrowTypeError(ctx, "zts: register_method expects a direct method function (not dispatch)");
    ...
    if (binding->memberKeys.find(name) != binding->memberKeys.end())
        return JS_ThrowTypeError(ctx, "zts: register_method conflict: %s", name.c_str());
    ...
    if (!JS_IsObject(target))
        return JS_ThrowTypeError(ctx, "zts: register_method: type binding incomplete");
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: make_generic_method requires method and type args");

    MethodMarshalCtx* openCtx = MetaBinding::TryGetDirectMethodCtx(ctx, argv[0]);
    if (openCtx == nullptr || openCtx->method == nullptr)
        return JS_ThrowTypeError(ctx, "zts: make_generic_method expects a direct method function (not dispatch)");
    const MethodInfo* openMethod = openCtx->method;
    if (!openMethod->is_generic)
        return JS_ThrowTypeError(ctx, "zts: make_generic_method requires an open generic method definition");

    JSValue argsVal = argv[1];
    if (!JS_IsArray(ctx, argsVal))
        return JS_ThrowTypeError(ctx, "zts: make_generic_method args must be an array");
    ...
    JS_FreeValue(ctx, lenVal);
    if (count < 0)
        return JS_ThrowTypeError(ctx, "zts: make_generic_method args length invalid");
    ...
    if (container == nullptr)
        return JS_ThrowTypeError(ctx, "zts: make_generic_method: missing generic container");
    const uint32_t expectedArgs = il2cpp::vm::MetadataCache::GetGenericContainerCount(container);
    if ((uint32_t)count != expectedArgs)
    {
        return JS_ThrowTypeError(
            ctx,
            "zts: make_generic_method expects %u type argument(s), got %d",
            expectedArgs,
            count);
    }
    ...
    for (int32_t i = 0; i < count; ++i)
    {
        ...
        JS_FreeValue(ctx, item);
        if (argKlass == nullptr)
            return JS_ThrowTypeError(ctx, "zts: invalid generic method type argument");
    ...
    if (closed == nullptr)
        return JS_ThrowTypeError(ctx, "zts: failed to inflate generic method");
    ...
    if (closedCtx == nullptr)
        return JS_ThrowTypeError(ctx, "zts: closed generic method not marshalable yet");
...
static JSValue CreateSignatureCallback(JSContext* ctx, JSValueConst /*this_val*/, int argc, JSValueConst* argv)
{
    ...
    for (int32_t i = 0; i < count; ++i)
    {
        ...
        if (klass == nullptr)
        {
            if (freeArgs)
                JS_FreeValue(ctx, argsVal);
            return JS_ThrowTypeError(ctx, "zts: create_signature expects type arguments");
        }
...
static bool ReadIntJsArray(JSContext* ctx, JSValueConst jsArray, std::vector<int32_t>& out, const char* label)
{
    ...
    {
        JS_ThrowTypeError(ctx, "zts: %s must be a JS Array", label);
        return false;
    }
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: make_mdarray_type requires element type and rank");
    Il2CppClass* element = KlassFromTypeObject(ctx, argv[0]);
    if (element == nullptr)
        return JS_ThrowTypeError(ctx, "zts: make_mdarray_type expects element type");
    int32_t rank = 0;
    if (JS_ToInt32(ctx, &rank, argv[1]))
        return JS_EXCEPTION;
    if (rank < 1 || rank > 32)
        return JS_ThrowRangeError(ctx, "zts: make_mdarray_type rank must be in [1, 32]");
    Il2CppClass* arrayKlass = il2cpp::vm::Class::GetArrayClass(element, (uint32_t)rank);
    if (arrayKlass == nullptr)
        return JS_ThrowInternalError(ctx, "zts: failed to create mdarray type");
    return TypeRegistry::PushTypeObject(ctx, arrayKlass);
}
...
{
    if (argc < 3)
        return JS_ThrowTypeError(ctx, "zts: new_mdarray_by_spec requires element, lowbounds, sizes");
    Il2CppClass* element = KlassFromTypeObject(ctx, argv[0]);
    if (element == nullptr)
        return JS_ThrowTypeError(ctx, "zts: expected element type");
    ...
    if (lowBounds.size() != sizes.size() || sizes.empty())
        return JS_ThrowTypeError(ctx, "zts: new_mdarray_by_spec: lowbounds/sizes rank mismatch");
...
{
    if (argc < 3)
        return JS_ThrowTypeError(ctx, "zts: new_mdarray_by_mdarray_type requires type, lowbounds, sizes");
    Il2CppClass* arrayKlass = KlassFromTypeObject(ctx, argv[0]);
    if (arrayKlass == nullptr || arrayKlass->rank < 1)
        return JS_ThrowTypeError(ctx, "zts: new_mdarray_by_mdarray_type expects mdarray type");
    std::vector<int32_t> lowBounds;
    std::vector<int32_t> sizes;
    if (!ReadIntJsArray(ctx, argv[1], lowBounds, "lowbounds") || !ReadIntJsArray(ctx, argv[2], sizes, "sizes"))
        return JS_EXCEPTION;
    if (lowBounds.size() != sizes.size() || (int32_t)sizes.size() != arrayKlass->rank)
        return JS_ThrowTypeError(ctx, "zts: new_mdarray_by_mdarray_type: lowbounds/sizes rank mismatch");
...
{
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "zts: get_opaquevalue requires a handle");
...
{
    if (argc < 2)
        return JS_ThrowTypeError(ctx, "zts: set_opaquevalue requires handle and value");
...
{
    if (argc < 1)
        return JS_ThrowTypeError(ctx, "zts: to_user_data requires an opaque handle");
...
void JsLib::RegisterGlobals(JSContext* ctx)
{
    ...
    BindGlobal(ctx, "__zts_new_szarray_by_element_type", NewSzArrayByElementCallback, 2);
