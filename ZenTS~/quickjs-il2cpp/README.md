...
| libc 替代 | `zents_qjs_std_stubs.c` + `quickjs-libc.h` |

## MSVC / Bee 适配（相对上游）

- `zents_il2cpp_config.h`：仅由各 `.c` 顶部 `#include`（**不要**再塞进 `quickjs.h`，以免与 winsock `timeval` 冲突）
...
- Dev：`zents_il2cpp_config.h` 把 QuickJS `abort()` 重定向到写 `zents_il2cpp_assert.log` + `_exit(3)`（无 CRT 弹窗）
